    ...
    <div className={`bg-white rounded-[34px] w-[95%] flex flex-col justify-between ${shadowClass} transition duration-200 shadow-md mx-auto`} id={`${id}`}>
        ...
        <div className={`flex flex-row mt-[20px] mx-[20px] justify-between skeleton-50`}>
          <div className={`rounded-[14px] bg-accentColor-100 border-accentColor-200 border-[2px]`}>
            {/* cardData?.label: The optional chaining (?.) ensures that if cardData is undefined, it won't attempt to read label and will avoid throwing an error. */}
            <p className={`px-[15px] py-[3px] text-accentColor-400`}>{ cardData?.label ? (cardData.label) : ('Label') }</p>
          </div>
          <div>
          </div>
        ...
        <div className={`h-11/12 min-h-[100px] bg-neutralGray-200/10 mt-[15px] mx-[13px] rounded-[21px] mb-[1.5em] skeleton`}>
          <p className={`text-[18px] text-gray/90 md:text-[20px] w-11/12 mx-auto mt-[0.75em] mb-[0.75em]`}>{ cardData?.description ? (cardData?.description) : ('Brak wiadomości') }</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between">
          <div className={`mb-[20px] mx-[20px] flex flex-row gap-x-2`}>
            ...
            <p className="text-gray-200/80 font-[400] text-[14px] lg:text-[16px] my-auto pt-[1px]">{ cardData?.date ? (cardData?.date) : ('---') }</p>
